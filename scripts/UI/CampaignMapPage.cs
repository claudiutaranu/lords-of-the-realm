...
public partial class CampaignMapPage : Control
{
	...
	private void OpenRoom(string scenePath, string verb)
	{
		...
		_room = GD.Load<PackedScene>(scenePath).Instantiate<ProductionPage>();
		AddChild(_room);
		_room.Open(economy);
		_room.Closed += () =>
		{
	...
	{
		// Two of them open onto a room of the province in hand rather than onto a panel of text.
